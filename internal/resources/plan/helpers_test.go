...
// TestFilterManagedExceptionSetEntries_RemovesManagedSets verifies managed exception sets are filtered out.
func TestFilterManagedExceptionSetEntries_RemovesManagedSets(t *testing.T) {
	t.Parallel()

	sets := []jamfprotect.PlanExceptionSet{
		{UUID: "uuid-1", Name: "Custom Exceptions", Managed: false},
		{UUID: "uuid-2", Name: "Managed Exceptions", Managed: true},
		{UUID: "uuid-3", Name: "Another Custom", Managed: false},
	}

	result := filterManagedExceptionSetEntries(sets)

	if len(result) != 2 {
		t.Fatalf("filterManagedExceptionSetEntries returned %d entries, want 2", len(result))
	}
	if result[0].UUID != "uuid-1" || result[1].UUID != "uuid-3" {
		t.Errorf("unexpected result: %v", result)
	}
}

// TestFilterManagedExceptionSetEntries_EmptyInput verifies an empty input returns nil.
func TestFilterManagedExceptionSetEntries_EmptyInput(t *testing.T) {
	t.Parallel()

	result := filterManagedExceptionSetEntries(nil)
	if result != nil {
		t.Errorf("expected nil for nil input, got %v", result)
	}

	result = filterManagedExceptionSetEntries([]jamfprotect.PlanExceptionSet{})
	if result != nil {
		t.Errorf("expected nil for empty input, got %v", result)
	}
}

// TestFilterManagedExceptionSetEntries_AllManaged verifies all managed sets returns an empty result.
func TestFilterManagedExceptionSetEntries_AllManaged(t *testing.T) {
	t.Parallel()

	sets := []jamfprotect.PlanExceptionSet{
		{UUID: "uuid-1", Name: "Managed A", Managed: true},
		{UUID: "uuid-2", Name: "Managed B", Managed: true},
	}

	result := filterManagedExceptionSetEntries(sets)

	if len(result) != 0 {
		t.Errorf("filterManagedExceptionSetEntries returned %d entries, want 0", len(result))
	}
}

// TestFilterManagedExceptionSetEntries_NoneManaged verifies all non-managed sets pass through.
func TestFilterManagedExceptionSetEntries_NoneManaged(t *testing.T) {
	t.Parallel()

	sets := []jamfprotect.PlanExceptionSet{
		{UUID: "uuid-1", Name: "Custom A", Managed: false},
		{UUID: "uuid-2", Name: "Custom B", Managed: false},
	}
